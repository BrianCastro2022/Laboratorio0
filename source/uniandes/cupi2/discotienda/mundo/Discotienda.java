...
public class Discotienda
{
	...
	/**
	 * @author BRIAN CASTRO
	 *  Se requiere generar un informe de discos en la discotienda.
	...
	/**
	 * Se requiere un servicio en la aplicación discotienda que permita generar reportes de todos los discos de los géneros ROCK y POP y además sea económico, es decir el precio total no supere los (1000) mil pesos. El informe debe contener todos los datos en el disco.
	 * @author BRIAN CASTRO
	 * @throws FileNotFoundException
	 */
	public void generarInformeDeTodosLosDiscos() throws FileNotFoundException
	{
		File archivo = new File("./data/discosCostosos.txt");


		List<Disco> discosLista = discos.stream().filter(miDisco -> (((Disco) miDisco).darGenero().equalsIgnoreCase("ROCK") || ((Disco) miDisco).darGenero().equalsIgnoreCase("POP"))
				&& ((Disco) miDisco).darPrecioDisco() > 1000).toList();

		if (discosLista.isEmpty()) {
			throw new FileNotFoundException("No se encontraron discos que cumplan las condiciones para el informe.");
		}


		try (PrintWriter pluma = new PrintWriter(archivo)) {
			pluma.println("Informe de todos los discos");
			pluma.println("_____________________________________");

			discosLista.forEach(miDisco -> {
				pluma.println("NOMBRE: " + miDisco.darNombreDisco());
				...
				pluma.println("______________________________________");
			});
		}
	}
	...
	public String metodo2( )
	{
		try {
			...
			generarInformeDeTodosLosDiscos();
